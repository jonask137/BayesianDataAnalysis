...
*This chapter introduce linear regression as a Bayesian procedure*. Thus we will apply probability measures for intepretation, as that is what Bayesian is.

Why is the chapter called geocentric models? That is because Richard starts with an example of a scientist predicting the approximate location of planets, by working on a means level.

When we apply the same trail of thoughts in applied statistics, we have linear regression, where we try to understand what the mean is constructed by and how it varies.

The model also elaborates on how we mathematically formulate models, and distributions are applied.
...
Up until now we have just seen how random events can be described by distributions and how we may model these to get a better understanding of different scenarios.

In this section we are going to see how one may take one collection of observations and make inference on how that effect another varieble, specifically the relationship between weight and height for the Kalahari foragers.
...
At first we have a very simple Gaussian model, that looks the following:
...
Now we can elaborate and add a linear model, as the above only has the likelihood (probability of the data), and the two priors.

To specify the linear model we will say:

*let x = weight, thus* $\bar{x}$ is the average weight parameter of the observed variable.

![](images/paste-604F2802.png)

We see three types of elements:

1. likelihood (probability of the data), Now we see that heights are stochastic and normally distributed and is described by the mean and standard deviation. To predict height, we need the mean of the weight variable, thus we have the subscript *i* for each mean. **Basically each height estimate ($h_i$) consists of a $\mu_i$. Thus expected height is now applied on a series of data.**
1. the linear model, The linear model is deterministic hence the `=` and not the `~` sign (which is a distribution). As the likelihood (probability of the data) is now dependent on each mu, we see that the linear model is dependent on the priors.
1. the priors, it consists of the priors (assumptions), where we express $\alpha$, $\beta$ and $\sigma$. These are all stochastic, where $\alpha$ and $\beta$ are normally distributed while $\sigma$ is uniform.

***Why do we subtract the mean from x?*** that is basically because we want to do inference on the mean, hence if $x = \bar{x}$, then we are left with the mean value ($\alpha$). Thus we are able to separate two things:

1. The slope, i.e., the rate of change in expectation.
2. The mean

Now one can simulate the priors, to see what assumptions we are giving the golem.
...
It is often easiest to deduct conclusions based on the plots. The following make examples of both the tables and the plots. Such benefits are:

1. Whehter or not the model fitting procedure worked correctly
1. The absolute magniturde, rather than merely relative magnitude of a relationship between outcome and predictor.
...
> What is `link` doing? Link is basically just mapping data onto the model. Hence it will recognize the variable names onto the model and execute the mathematical model that you have specified. Thus basically just writing out the functions and executing these will also applying the priors that you have specified. If no data is provided it will merely use `extract.samples()`, to create data.
...
Now what we want to do is the same, but for *all weights*. For this one can use the `link` function. Notice that we did not specify what data, hence it will merely use `extract.samples()`.
...
Now we see that since we fed 46 values for the weight, we get 46 columns instead of one pr individual.
...
We see that the linear model does not fit the data well enough in the outer regions.
...
#### The same but using log of weight
...
The conclusion is that the data clearly need to be mapped with a non linear funciton.
